...
for (const channel of ['chrome', 'msedge']) {
  ...
  try {
    ...
    for (const id of ['audio-file', 'start-ms', 'end-ms', 'target-duration', 'pitch-shift', 'source-play', 'result-play', 'export-wav', 'result-loop', 'navigation-help']) assert.ok(tabOrder.includes(id), `${channel}: Tab missed ${id}`);
    await page.locator('#target-duration').fill('10'); await page.waitForFunction(() => !document.querySelector('#result-play').disabled);
    await page.locator('#result-play').focus(); await page.keyboard.press('Space'); assert.equal(await page.locator('#result-audio').evaluate(a => a.paused), false); await page.locator('#result-play').click();
    ...
    assert.ok((await page.locator('#file-info').textContent()).includes('3601.000')); assert.ok((await page.locator('#parameter-error').textContent()).includes('Shorten')); assert.equal(await page.locator('#stretch').isDisabled(), true);
    ...
    await page.locator('#stretch').fill('500'); await page.locator('#pitch-shift').fill('12'); await page.locator('#reverse-source').check(); await page.waitForFunction(() => !document.querySelector('#result-play').disabled); assert.equal(await page.locator('#result-audio').evaluate(a => a.duration), 5);
    await page.locator('#audio-file').setInputFiles({ name: 'empty.wav', mimeType: 'audio/wav', buffer: Buffer.alloc(0) }); await page.waitForFunction(() => !document.querySelector('#audio-file').disabled); assert.ok((await page.locator('#load-error').textContent()).includes('empty'));
