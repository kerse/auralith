...
for (const channel of ['chrome', 'msedge']) {
  ...
  try {
    ...
    await page.locator('#preview').click(); await page.waitForFunction(() => !document.querySelector('#result-play').disabled); await page.waitForFunction(() => Number.isFinite(document.querySelector('#result-audio').duration) && document.querySelector('#result-audio').duration > 0);
    ...
    assert.equal(await page.locator('#status').textContent(), 'Processing cancelled');
    await page.locator('#preview').click(); await page.waitForFunction(() => !document.querySelector('#result-play').disabled); await page.waitForFunction(() => Number.isFinite(document.querySelector('#result-audio').duration) && document.querySelector('#result-audio').duration > 0);
