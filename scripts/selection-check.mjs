...
for (const channel of ['chrome', 'msedge']) {
  ...
  try {
    ...
    assert.equal(await page.locator('#status').textContent(), 'Parameters changed. Press Preview for a new listen.');
    ...
    await page.locator('#source-play').click(); await page.waitForTimeout(100); assert.ok((await page.locator('#source-play').textContent()).includes('Stop source')); await page.locator('#source-play').click();
