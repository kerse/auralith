...
export class ExportControls {
  constructor(results) {
    ...
    document.querySelector('#export-slot').innerHTML = '<button id="export-wav" class="secondary" disabled>Export WAV</button>';
  ...
  async run() {
    ...
    try {
      ...
      if (!window.showSaveFilePicker && size > BLOB_LIMIT) throw new Error('Для WAV больше 128 МБ нужен браузер с сохранением напрямую на диск (Chrome/Edge). Уменьшите длительность или откройте приложение в поддерживаемом браузере.');
      r.stop(); r.stopSource(); r.lock(true);
      r.cancelOverride = () => { cancelled = true; worker?.terminate(); rejectJob?.(abortError()); };
      document.querySelector('#progress').value = 0; document.querySelector('#progress-label').textContent = 'Выбор файла…';
      const parts = [];
      ...
      if (cancelled) throw abortError();
      await writer.write(header); setStatus('Экспорт: обработка аудио…');
      ...
      await new Promise((resolve, reject) => {
        ...
        worker.onerror = () => reject(new Error('Ошибка экспортного worker. Возможно, недостаточно памяти.'));
      ...
      if (cancelled) throw abortError();
      if (bytesWritten !== size) throw new Error('Размер WAV не совпал с ожидаемым. Экспорт прерван.');
      document.querySelector('#progress-label').textContent = 'Сохранение WAV…'; document.querySelector('#progress').value = 1;
      ...
      if (parts.length) {
        ...
        setStatus(`Скачивание начато: ${name} · ${(size / 1048576).toFixed(1)} МБ`);
      } else setStatus(`WAV сохранён: ${name} · ${(size / 1048576).toFixed(1)} МБ`);
    } catch (error) {
      setStatus(error.name === 'AbortError' ? 'Экспорт отменён' : `Не удалось экспортировать WAV: ${error.message}. Проверьте свободное место и попробуйте меньшую длительность.`, error.name !== 'AbortError');
    } finally {
