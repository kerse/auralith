...
export class ResultControls {
  constructor({ getSource, getParameters, setBusy, stopSource }) {
    ...
    document.querySelector('#result-controls').innerHTML = `<div class="result-actions"><button id="preview" disabled>Preview</button><button id="cancel-render" class="secondary" hidden>Отменить</button><span id="export-slot"></span></div><div id="render-progress" hidden><label for="progress">Обработка</label><progress id="progress" max="1" value="0"></progress><span id="progress-label" role="status">0%</span></div><p id="preview-note" class="hint">Загрузите звук, выберите фрагмент и нажмите Preview.</p><audio id="result-audio" preload="auto"></audio><div class="player"><button id="result-play" disabled>▶ Play</button><button id="result-stop" class="secondary" disabled>■ Stop</button><label><input id="result-loop" type="checkbox" checked> Loop</label><span id="result-time">00:00.0 / 00:00.0</span></div>`;
    ...
    document.querySelector('#result-play').onclick = async () => { try { if (this.audio.paused) await this.audio.play(); else this.audio.pause(); } catch (error) { setStatus(`Не удалось воспроизвести: ${error.message}`, true); } };
    ...
    for (const event of ['timeupdate', 'loadedmetadata', 'play', 'pause', 'ended']) this.audio.addEventListener(event, () => {
      document.querySelector('#result-play').textContent = this.audio.paused ? '▶ Play' : 'Ⅱ Pause';
  ...
  invalidate() {
    ...
    document.querySelector('#preview-note').textContent = 'Параметры изменены. Нажмите Preview для нового прослушивания.';
    // The footer is global. Clear an error left by an earlier render once the
    // user changes the input that will be rendered next.
    if (this.getSource()) setStatus('Параметры изменены. Нажмите Preview для нового прослушивания.');
    this.refresh();
  }
  ...
  async preview() {
    ...
    try {
      ...
      if (!source) throw new Error('Загрузите звук.');
      ...
      const data = await new Promise((resolve, reject) => {
        ...
        worker.onerror = () => reject(new Error('Ошибка worker. Возможно, недостаточно памяти. Попробуйте меньший фрагмент.'));
      ...
      document.querySelector('#preview-note').textContent = options.duration > 20 ? 'Черновой Preview: первые 20 секунд результата. Экспорт рассчитает всю длительность с полным качеством.' : 'Черновой Preview · FFT 2048. Экспорт будет рассчитан отдельно с полным качеством.';
      setStatus('Preview готов');
    } catch (error) { setStatus(error.name === 'AbortError' ? 'Обработка отменена' : error.message, error.name !== 'AbortError'); }
