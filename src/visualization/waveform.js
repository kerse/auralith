...
export class Waveform {
  constructor(onSelection = () => {}) {
    ...
    document.querySelector('#visualizations').innerHTML = `<div id="wave-panel" hidden><div class="view-toolbar"><span>Форма волны</span><label>Масштаб <input id="zoom" type="range" min="1" max="32" step="1" value="1"></label><label class="scroll-label">Прокрутка <input id="scroll" type="range" min="0" max="1" step=".001" value="0" disabled></label></div><canvas id="waveform" height="200" aria-label="Форма волны. Для точного выделения используйте поля начала и конца ниже."></canvas><div id="spectrogram-slot"></div><p class="hint">Тяните границы или выделение целиком. Щёлкните и потяните вне выделения, чтобы выбрать новый фрагмент.</p></div>`;
    this.canvas = document.querySelector('#waveform');
    document.querySelector('#selection-controls').innerHTML = `<div id="selection-panel" hidden><div class="selection-row">${['start', 'end'].map((bound, i) => `<fieldset><legend>${i ? 'Конец' : 'Начало'}</legend>${[['min', 'мин', 59], ['sec', 'сек', 59], ['ms', 'мс', 999]].map(([unit, label, max]) => `<label><input id="${bound}-${unit}" type="number" min="0" max="${unit === 'min' ? 9999 : max}" step="1" value="0"> ${label}</label>`).join('')}</fieldset>`).join('')}<button id="source-play" type="button">▶ Исходный фрагмент</button><span id="selection-length" class="muted"></span></div><p id="selection-error" class="error" role="alert" hidden></p></div>`;
  ...
  async load(source) {
    ...
    const peaks = await new Promise((resolve, reject) => {
      ...
      worker.onerror = () => { worker.terminate(); reject(new Error('Не удалось построить waveform.')); };
  ...
  updateFields() {
    ...
    document.querySelector('#selection-length').textContent = duration < .001 ? `Фрагмент: ${(duration * 1000).toFixed(3)} мс · ${Math.max(1, Math.round(duration * this.source.sampleRate))} отсч.` : `Фрагмент: ${duration.toFixed(3)} с`;
  }
  readFields() {
    ...
    if (!Number.isFinite(start + end) || rawStart < 0 || rawEnd > this.source.duration + .0000005 || end > this.source.duration || Math.round((end - start) * this.source.sampleRate) < 1) { error.textContent = 'Начало должно быть раньше конца, а диапазон — внутри файла. На графике сохранено последнее корректное выделение.'; error.hidden = false; return; }
  ...
  draw() {
    ...
    this.peaks.forEach(({ min, max }, ch) => {
      ...
      ctx.stroke(); ctx.fillStyle = '#28584c'; ctx.font = '12px Consolas'; ctx.fillText(this.peaks.length === 1 ? 'MONO' : ch ? 'R' : 'L', 8, mid - lane * .3);
    });
  ...
  async play() {
    ...
    if (!document.querySelector('#selection-error').hidden) throw new Error('Исправьте время выделения перед прослушиванием.');
    ...
    node.start(0, this.start, this.end - this.start); document.querySelector('#source-play').textContent = '■ Остановить исходник';
  }
  stop() { if (this.node) { this.node.onended = null; this.node.stop(); this.node.disconnect(); this.node = null; } const b = document.querySelector('#source-play'); if (b) b.textContent = '▶ Исходный фрагмент'; }
}
